...
class FrameController extends Controller
{
  public function getPricing(Request $request)
  {
    ...
    foreach ($regulars as $regular) {
      $regular['name'] = $regular['frame_dimension'] . '(NGN' . $regular['price'] . ')';
    }

    $illustrations = FrameDimension::where('frame_type_id', 2)->get();
    foreach ($illustrations as $illustration) {
      $illustration['name'] = $illustration['frame_dimension'] . '(NGN' . $illustration['price'] . ')';
    }


    $quotes = FrameDimension::where('frame_type_id', 3)->get();
    foreach ($quotes as $quote) {
      $quote['name'] = $quote['frame_dimension'] . '(NGN' . $quote['price'] . ')';
    }
